...
class TimelineWatch(ndb.Model):
  """A Mastodon API OAuth2 app registered with a specific instance."""
  id = ndb.IntegerProperty(required=True)  # URL, eg https://mastodon.social/
  instance = ndb.StringProperty(required=True)  # URL, eg https://mastodon.social/
  url = ndb.StringProperty(required=True)  # JSON; includes client id/secret
  content = ndb.TextProperty()  # JSON; from /api/v1/instance
  created_at = ndb.DateTimeProperty(auto_now_add=True, required=True)
  last_checked_at = ndb.DateTimeProperty(auto_now_add=True, required=True)

class ReplyCheck(ndb.Model):
  """A Mastodon API OAuth2 app registered with a specific instance."""
  id = ndb.IntegerProperty(required=True)  # URL, eg https://mastodon.social/
  instance = ndb.StringProperty(required=True)  # URL, eg https://mastodon.social/
  url = ndb.StringProperty(required=True)  # JSON; includes client id/secret
  content = ndb.TextProperty()  # JSON; from /api/v1/instance
  report = ndb.BooleanProperty()
  score = ndb.TextProperty()
  covered = ndb.BooleanProperty()
  created_at = ndb.DateTimeProperty(auto_now_add=True, required=True)
  last_checked_at = ndb.DateTimeProperty(auto_now_add=True, required=True)
